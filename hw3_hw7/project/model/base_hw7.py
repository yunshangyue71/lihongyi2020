...
class StudentNet(nn.Module):
    ...
    def __init__(self, bandwidth=[64,64, 128, 128, 256,256, 512,512, 1024,1024], width_mult=1):
        '''
          Args:
...
class StudentNetOffice(nn.Module):
    '''
      在這個Net裡面，我們會使用Depthwise & Pointwise Convolution Layer來疊model。
      你會發現，將原本的Convolution Layer換成Dw & Pw後，Accuracy通常不會降很多。
    ...
    def __init__(self, bandwidth=[16, 32, 64, 128, 256, 256, 256, 256], width_mult=1):
        '''
          Args:
            base: 這個model一開始的ch數量，每過一層都會*2，直到base*16為止。
            width_mult: 為了之後的Network Pruning使用，在base*8 chs的Layer上會 * width_mult代表剪枝後的ch數量。        
        '''
        super(StudentNetOffice, self).__init__()
        # multiplier = [1, 2, 4, 8, 16, 16, 16, 16]
