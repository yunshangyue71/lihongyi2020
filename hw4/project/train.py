...
def training(batch_size, n_epoch, lr, model_dir, train, valid, model, device):
    ...
    for epoch in range(n_epoch):
        ...
        for i, (inputs, labels) in enumerate(train):
            ...
            label_index = torch.where(labels >= 0)
            unlable_index = torch.where(labels == -1)

            ux = outputs[unlable_index].cuda()#.type(torch.LongTensor).cuda()

            loss_c = -(ux * torch.log(ux) + (1-ux) * torch.log(1-ux)).sum() * 0.03
            loss = criterion(outputs[label_index], labels[label_index]) + 0.0001*L2(model)  + loss_c# 計算此時模型的 training loss
            ...
            # print(i)
            print('[ Epochs{}: {}/{} ] loss:{:.3f} acc:{:.3f} '.format(
